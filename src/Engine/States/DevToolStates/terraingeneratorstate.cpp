#include "terraingeneratorstate.h"

TerrainGeneratorState TerrainGeneratorState::m_TerrainGeneratorState;
//EventHandlingStateBase * EventHandlingStateBase::event_handling_instance;

void TerrainGeneratorState::Init(GrafixEngine *game) {
    ...
    terraingenerator.Init(game);
};

void TerrainGeneratorState::Cleanup()
{
    ...
    terraingenerator.Cleanup(); // Cleanup World Builder
};

void TerrainGeneratorState::Pause()
{

};

void TerrainGeneratorState::Resume(GrafixEngine* game)
{

};

void TerrainGeneratorState::HandleEvents(GrafixEngine* game)
{
    glfwPollEvents();
    ...
    if (bID==0)
    {
        ...
        terraingenerator.UpdateEvents(input);
    }
...
void TerrainGeneratorState::Update(GrafixEngine* game)
{
    dt=glfwGetTime()-dt;
...
void TerrainGeneratorState::Draw(GrafixEngine* game)
{
    // Render
    ...
    // Draw Menus
    SW.RenderTextCentered("TerrainGenerator",1,0.0,1,0.92,1.5,glm::vec3(1.0));
...
void TerrainGeneratorState::key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
...
void TerrainGeneratorState::mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    if(action == GLFW_PRESS)
...
void TerrainGeneratorState::mouse_pos_callback(GLFWwindow* window, double xpos, double ypos)
{
...
void TerrainGeneratorState::scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
...
void TerrainGeneratorState::cursor_enter_callback(GLFWwindow* window, int entered)
{
