...
*/
void DropDownMenu::Init(std::string title,double x, double y,float length,float height,float scale,
...
*/
void DropDownMenu::SetColors(glm::vec3 c1,glm::vec3 c2,glm::vec4 c3,glm::vec4 c4,glm::vec4 c5,glm::vec4 c6)
{
    textcolor[0]=c1;
    textcolor[1]=c2;
...
*/
void DropDownMenu::SetSelection(int i)
{
    selection=i;
};
...
*/
void DropDownMenu::UpdateEvents(InputStruct &input)
{
    double xmp,ymp;
...
active if it is.
*/
bool DropDownMenu::CheckMouseOver(float xmp,float ymp)
{
    // Check X over
    bool xchk=false;
...
active if it is.
*/
int DropDownMenu::CheckMouseOverSelection(float xmp,float ymp)
{
    int rtnmo=selection;
    for(int i=0; i<(int)data.size(); ++i)
    {
...
*/
int DropDownMenu::SelectionState()
{
    // Return Result
    int slc=selection;
    selection=-1;
